...
export const GET = async (req, { params }) => {
  try {
    await connectDB();
    const arrangement = await Arrangement.findById(params.id).populate('creator');
    if (!arrangement) return new Response('Arrangement not found.', { status: 404 });
...
export const PUT = async (req, { params }) => {
  try {
    ...
    await connectDB();
    const res = await Arrangement.findByIdAndUpdate(params.id, updatedArrangement, {
      new: true,
      runValidators: true,
    });
...
export const DELETE = async (req, { params }) => {
  try {
    ...
    if (!deletedArrangement) return new Response('Arrangement not found.', { status: 404 });
